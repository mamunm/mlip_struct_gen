...
class MetalSaltWaterLAMMPSGenerator(BaseLAMMPSGenerator):
    ...
    def _generate_initialization_section(self) -> list[str]:
        ...
        # Header comments
        lines.append(
            f"# LAMMPS Input File: {self.parameters.metal_type}-{self.parameters.salt_type}-Water {self.parameters.ensemble} at {self.parameters.temperatures[0]}K"
        )
        lines.append(
            f"# Generated by mlip-struct-gen on {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        )
        lines.append(f"# Atom types: 1={self.parameters.metal_type}, 2=O, 3=H, 4={self.cation}, 5={self.anion}")
        ...
        if self.parameters.metal_type in self.metal_lj_params:
            ...
            lines.append(f"# LJ potential: sigma={lj['sigma']:.3f} A, epsilon={lj['epsilon']:.6f} eV")
        lines.append("")
    ...
    def _generate_force_field_section(self, temperature: float) -> list[str]:
        ...
        # Define potentials
        lines.append(f"# Define potentials for {self.parameters.metal_type}-{self.parameters.salt_type}-water system")
        lines.append(f"# {self.parameters.water_model} water parameters for metal units (eV, Angstrom)")
        lines.append(f"pair_style lj/cut/coul/long 10.0 10.0")
        lines.append("")

        # Metal-metal interactions
        if self.parameters.metal_type in self.metal_lj_params:
            metal_lj = self.metal_lj_params[self.parameters.metal_type]
            lines.append(f"# {self.parameters.metal_type}-{self.parameters.metal_type} interactions (LJ potential)")
            lines.append(
        ...
        lines.append(f"# {self.parameters.water_model} water interactions (converted to metal units)")
        ...
        lines.append(f"# Charges are already defined in data file")
        lines.append(f"# Water {self.parameters.water_model}: O: {self.water_params['O_charge']:.4f} e, H: {self.water_params['H_charge']:.4f} e")
        lines.append(f"# Ions: {self.cation}: {self.cation_params['charge']:.1f} e, {self.anion}: {self.anion_params['charge']:.1f} e")
        lines.append("")
    ...
    def _generate_settings_section(self) -> list[str]:
        ...
        lines.append("fix shake all shake 0.0001 20 0 b 1 a 1          # constrain O-H bonds and H-O-H angles")
        lines.append("")
    ...
    def _generate_equilibration_section(self, temperature: float) -> list[str]:
        ...
        lines.append(f"timestep {timestep_ps:.3f}                                     # {self.parameters.timestep:.1f} fs timestep")
        lines.append("")
        ...
        lines.append("thermo_style custom step time temp c_temp_water c_temp_solution pe ke etotal press vol density")
        lines.append('thermo_modify colname c_temp_water "T_water" colname c_temp_solution "T_soln"')
        ...
        lines.append(f"thermo {thermo_out}                                        # output every {thermo_out} steps")
        lines.append("")
    ...
    def _generate_production_section(self, temperature: float) -> list[str]:
        ...
        if self.parameters.compute_stress:
            ...
            lines.append(f"dump forces all custom {dump_steps} forces.dump id type x y z fx fy fz c_stress_atom[*]")
            lines.append("")

        # Restart files
        lines.append("# Restart files")
        lines.append(f"restart {restart_steps} restart1.lmp restart2.lmp           # restart files every {restart_steps} steps")
        lines.append("")
        ...
        lines.append(f'print "Simulation completed: {self.parameters.production_time:.1f} ps {self.parameters.ensemble} at {temperature:.1f}K"')
        lines.append("")
