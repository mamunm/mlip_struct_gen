...
class MetalWaterLAMMPSGenerator(BaseLAMMPSGenerator):
    ...
    def _generate_initialization_section(self) -> list[str]:
        ...
        # Header comments
        lines.append(
            f"# LAMMPS Input File: {self.parameters.metal_type}-Water {self.parameters.ensemble} at {self.parameters.temperatures[0]}K"
        )
        lines.append(
    ...
    def _generate_force_field_section(self, temperature: float) -> list[str]:
        ...
        if self.parameters.metal_type in self.metal_lj_params:
            ...
            lines.append(f"pair_coeff 1 3 0.0100 {mh_sigma:.3f}    # {self.parameters.metal_type}-H: weak interaction")
        else:
        ...
        lines.append(f"# Charges are already defined in data file for {self.parameters.water_model} water")
        lines.append(f"# O: {self.water_params['O_charge']:.4f} e, H: {self.water_params['H_charge']:.4f} e")
        lines.append("")
    ...
    def _generate_equilibration_section(self, temperature: float) -> list[str]:
        ...
        lines.append("thermo_style custom step time temp c_temp_water pe ke etotal press vol density")
