...
def convert_to_raw(input_dir: str, output_dir: str, dry_run: bool = False) -> None:
    """
    ...
    logger.success(
        f"Successfully converted {len(outcar_dirs)} snapshots to raw format in {output_path}"
    )
    logger.info("Output files:")
    logger.info("  - type.raw (atom types)")
    logger.info("  - type_map.raw (element names)")
    logger.info("  - coord.raw (coordinates)")
    logger.info("  - force.raw (forces)")
    logger.info("  - energy.raw (energies)")
    if all_boxes:
        logger.info("  - box.raw (cell parameters)")
    if all_virials:
        logger.info("  - virial.raw (virial tensors)")


def convert_to_raw_batch(input_dirs: list[str], output_base_dir: str, **kwargs) -> None:
    """
