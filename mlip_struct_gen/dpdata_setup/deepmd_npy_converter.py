...
def convert_to_dpdata(input_dir: str, 
                      output_dir: str,
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for idx, outcar_dir in enumerate(outcar_dirs):
            ...
            try:
                ...
                if successful_conversions == 0 and actual_set_name is None:
                    ...
                    set_dirs = [d for d in temp_deepmd.iterdir() if d.is_dir() and d.name.startswith("set.")]
                    if not set_dirs:
        ...
        if successful_conversions == 0:
            raise RuntimeError(f"All {len(outcar_dirs)} OUTCAR conversions failed. No dpdata output created.")
    ...
    if successful_conversions > 0:
        logger.success(f"Successfully converted {successful_conversions} snapshots to {output_path}")
        logger.info(f"Output directory structure:")
        logger.info(f"  {output_path}/")
        logger.info(f"    ├── type.raw")
        logger.info(f"    ├── type_map.raw")
        logger.info(f"    └── {actual_set_name}/")
        logger.info(f"        ├── coord.npy")
        logger.info(f"        ├── energy.npy")
        logger.info(f"        ├── force.npy")
        logger.info(f"        └── box.npy (if periodic)")


def convert_to_dpdata_batch(input_dirs: List[str],
                           output_base_dir: str,
                           **kwargs) -> None:
    """
