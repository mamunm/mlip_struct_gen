...
class WaterBoxGenerator:
    ...
    def run(self, save_artifacts: bool = False) -> str:
        ...
        try:
            ...
            if self.parameters.output_format == "lammps":
                ...
                # Convert XYZ to LAMMPS atomic format for DPMD
                if self.logger:
                    self.logger.step("Converting XYZ to LAMMPS atomic format for DPMD")
                self._convert_xyz_to_lammps_atomic(temp_output, output_path)
    ...
    def _convert_xyz_to_lammps_atomic(self, input_xyz: Path, output_file: Path) -> None:
        """
        Convert XYZ file to LAMMPS data format with atomic style (no charges, bonds, angles).
        This format is suitable for DPMD simulations.
        ...
        with open(output_file, "w") as f:
            f.write(
                f"# LAMMPS data file for {self.parameters.water_model} water box (atomic style for DPMD)\n"
            )
            f.write("# Generated by mlip-struct-gen\n\n")

            # Counts section - only atoms, no bonds or angles
            f.write(f"{n_atoms} atoms\n")
            f.write("\n")
            ...
            for atom_id, (element, x, y, z) in enumerate(atoms, 1):
                atom_type = o_type if element == "O" else h_type
                f.write(f"{atom_id:6d} {atom_type:6d} " f"{x:12.6f} {y:12.6f} {z:12.6f}\n")

        if self.logger:
            self.logger.success(
                f"Successfully converted to LAMMPS atomic format for DPMD ({n_atoms} atoms)"
            )
