"""Walled metal-water interface generation using ASE and PACKMOL."""
...
try:
    ...
    raise ImportError(
        "ASE (Atomic Simulation Environment) is required for walled metal-water generation. "
        "Install with: pip install ase"
    ) from e


class WalledMetalWaterGenerator:
    """Generate walled FCC(111) metal surfaces with water between them using ASE and PACKMOL."""

    def __init__(self, parameters: WalledMetalWaterParameters):
        """
        Initialize walled metal-water interface generator.
        ...
        if self.logger:
            self.logger.info("Initializing WalledMetalWaterGenerator")
    ...
    def generate(self) -> str:
        """
        Generate the walled metal-water interface.
        ...
        try:
            ...
            error_msg = f"Failed to generate walled metal-water interface: {e}"
            if self.logger:
    ...
    def _build_metal_walls(self) -> None:
        ...
        # Build the FULL slab (all nz layers)
        full_slab = fcc111(
        ...
        # Bottom wall top z-coordinate
        self.bottom_wall_top = z_unique[self.n_bottom - 1]

        # Compute water height for positioning
        self.water_height = self._calculate_water_height()

        # Top wall z = bottom of the top wall
        self.top_wall_z = (
        ...
        # Shift top wall atoms to their new position
    ...
    def _apply_symmetric_constraints(self) -> None:
        ...
        # Bottom wall: fix the lowest n_fix layers
        bottom_fix_threshold = z_unique[n_fix - 1] + 0.01
        bottom_fixed = z_positions < bottom_fix_threshold

        # Top wall: fix the highest n_fix layers (outermost layers)
    ...
    def _calculate_water_height(self) -> float:
        ...
        # Convert density to g/Angstrom^3
        ...
        if self.logger:
            self.logger.info(
                ...
                f"at {self.parameters.density} g/cm^3: {water_height:.2f} Angstroms"
            )
    ...
    def _generate_water(self, tmpdir: str) -> None:
        """
        Generate water configuration using PACKMOL between the two walls.
        ...
        water_z_min = self.bottom_wall_top + self.parameters.gap_above_metal + margin_z_bottom
        water_z_max = (
            self.bottom_wall_top
            + self.parameters.gap_above_metal
            + self.water_height
...
"""
    ...
    def _combine_metal_water(self) -> None:
        """Combine metal walls and water molecules."""
        if self.metal_slab is None or self.water_atoms is None:
            raise ValueError("Both metal walls and water must be generated first")
    ...
    def _write_poscar(self, output_path: Path) -> None:
        ...
        with open(output_path, "w") as f:
            f.write(f"{self.parameters.metal}(111) walled surface with {n_o} water molecules\n")
            f.write("1.0\n")
            for i in range(3):
    ...
    def _write_lammps(self, output_path: Path) -> None:
        ...
        with open(output_path, "w") as f:
            f.write(
                f"LAMMPS data file for {self.parameters.metal}(111) walled surface with "
                f"{n_water} {self.parameters.water_model} water molecules\n\n"
            )
            ...
            for i in range(len(symbols)):
                if symbols[i] == self.parameters.metal:
                    f.write(
                        f"{atom_id} {mol_id} {metal_type} 0.0 "
            ...
            for i in range(len(symbols)):
                if symbols[i] == "O":
                    ...
                    f.write(
                        f"{atom_id} {mol_id} {o_type} {o_charge:.4f} "
                        f"{positions[i,0]:.6f} {positions[i,1]:.6f} {positions[i,2]:.6f}\n"
                    )
                    atom_id += 1
                elif symbols[i] == "H":
                    ...
                    f.write(
                        f"{atom_id} {mol_id} {h_type} {h_charge:.4f} "
                        f"{positions[i,0]:.6f} {positions[i,1]:.6f} {positions[i,2]:.6f}\n"
                    )
                    atom_id += 1
                    if len(h_atoms) % 2 == 0:
    ...
    def _write_lammpstrj(self, output_path: Path) -> None:
        ...
        with open(output_path, "w") as f:
            ...
            for i, (symbol, pos) in enumerate(zip(symbols, positions, strict=False)):
                ...
                f.write(f"{i+1} {atom_type} {symbol} " f"{pos[0]:.6f} {pos[1]:.6f} {pos[2]:.6f}\n")
    ...
    def _write_lammps_atomic(self, output_path: Path) -> None:
        ...
        with open(output_path, "w") as f:
            f.write(
                f"# LAMMPS data file for {self.parameters.metal}-water walled interface "
                f"(atomic style for DPMD)\n"
            )
            ...
            if self.parameters.elements:
                ...
                if self.parameters.metal in unique_elements:
                    ...
                    unique_elements = [self.parameters.metal] + unique_elements
        ...
        if self.logger:
            ...
            self.logger.info(
                f"System: {n_metal} {self.parameters.metal} atoms, " f"{n_water} water molecules"
            )
