"""Parameter validation for constrained metal-water interface generation."""
...
def validate_parameters(parameters: "ConstrainedMetalWaterParameters") -> None:
    """
    Validate and normalize parameters for constrained metal-water interface generation.
    ...
    if parameters.gap_above_metal < 0:
        raise ValueError("gap_above_metal must be non-negative")
    ...
    # Water-only distance constraints validation
    for constraint in parameters.distance_constraints:
        ...
        if constraint.element1 not in VALID_WATER_ELEMENTS:
            raise ValueError(
                f"Invalid element: {constraint.element1}. "
                f"Must be one of: {sorted(VALID_WATER_ELEMENTS)}"
            )
        if constraint.element2 not in VALID_WATER_ELEMENTS:
            raise ValueError(
