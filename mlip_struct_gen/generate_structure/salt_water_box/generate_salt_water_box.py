...
class SaltWaterBoxGenerator:
    ...
    def run(self, save_artifacts: bool = False) -> str:
        ...
        try:
            ...
            if self.parameters.output_format == "lammps":
                ...
                if self.logger:
                    self.logger.step("Converting XYZ to LAMMPS atomic format for DPMD")
                self._convert_xyz_to_lammps_atomic(temp_output, output_path, n_water)
    ...
    def _convert_xyz_to_lammps_atomic(
        self, input_xyz: Path, output_file: Path, n_water: int
    ) -> None:
        """
        Convert XYZ to LAMMPS atomic format (no charges, bonds, angles).
        ...
        # Parse atom positions and elements
        atoms = []
        element_counts = {}
        for i in range(2, 2 + n_atoms):
            parts = lines[i].strip().split()
        ...
        with open(output_file, "w") as f:
            f.write(
                f"# LAMMPS data file for {self.parameters.salt_type} salt water (atomic style for DPMD)\n"
            )
            f.write("# Generated by mlip-struct-gen\n\n")
            ...
            if self.parameters.elements:
                ...
                unique_elements = set(element_counts.keys())
                for elem in unique_elements:
                    if elem not in element_to_type:
                        element_to_type[elem] = len(element_to_type) + 1
                max_atom_type = len(self.parameters.elements)
            else:
                # Default order: O, H, cation, anion
                unique_elements = sorted(set(element_counts.keys()), reverse=True)  # O before H
            ...
            for atom_id, (element, x, y, z) in enumerate(atoms, 1):
                atom_type = element_to_type[element]
                f.write(f"{atom_id:6d} {atom_type:6d} {x:12.6f} {y:12.6f} {z:12.6f}\n")

        if self.logger:
            self.logger.success("Successfully converted to LAMMPS atomic format for DPMD")
            self.logger.info(
                f"System: {n_water} water, {self.n_cations} {cation_element}, {self.n_anions} {anion_element}"
            )
    ...
    def _convert_xyz_to_lammpstrj(self, xyz_file: Path, output_file: Path) -> None:
        ...
        with open(output_file, "w") as f:
            ...
            # Create type mapping - use elements list if provided, otherwise sort alphabetically
            if self.parameters.elements:
