"""CLI interface for spring-restrained metal-salt-water interface generation."""

import argparse
import sys
from pathlib import Path

from ...generate_structure.spring_metal_salt_water import (
...
def add_parser(subparsers: argparse._SubParsersAction) -> None:
    """Add the spring-metal-salt-water subcommand parser."""
    parser = subparsers.add_parser(
        "spring-metal-salt-water",
        help="Generate metal-salt-water interface with spring bond restraints",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        description="Generate spring-restrained metal-salt-water interface structures for MLIP training",
        epilog="""
Examples:
  1. Add spring restraint to Pt-O and Pt-Na distances:
     mlip-struct-gen generate spring-metal-salt-water \\
         --metal Pt --size 4 4 4 --n-water 50 \\
         --salt-type NaCl --n-salt 5 \\
         --spring-metal-water O 5 2.5 \\
         --spring-metal-ion Na 3 2.8 \\
         --type-map Pt O H Na Cl --model graph.000.pb \\
         --output pt_nacl_water_spring.data

  2. Full system with multiple spring restraints:
     mlip-struct-gen generate spring-metal-salt-water \\
         --metal Cu --size 4 4 4 --n-water 50 \\
         --salt-type NaCl --n-salt 5 \\
         --spring-metal-water O 3 2.3 \\
         --spring-metal-ion Na 2 2.5 \\
         --spring Na O 2 2.3 \\
         --spring O H 2 1.02 \\
         --k-spring 100 \\
         --type-map Cu O H Na Cl --model graph.000.pb \\
         --output cu_nacl_water_spring.data
        """,
    )
    ...
    # Metal-ion spring constraint arguments
    parser.add_argument(
        "--spring-metal-ion",
        nargs=3,
        action="append",
        metavar=("ION_ELEM", "COUNT", "DISTANCE"),
        help="Metal-ion spring: Na 3 2.8 (3 Metal-Na pairs at 2.8 A)",
    )

    # General spring constraint arguments
    parser.add_argument(
        "--spring",
        nargs=4,
        action="append",
...
def validate_args(args: argparse.Namespace) -> None:
    ...
    has_constraints = args.spring_metal_water or args.spring_metal_ion or args.spring
    if not has_constraints:
        logger.error(
            "Must specify at least one spring constraint "
            "(--spring-metal-water, --spring-metal-ion, or --spring)"
        )
        sys.exit(1)
...
def get_default_elements(metal: str, salt_type: str) -> list[str]:
    ...
    ions = salt_elements.get(salt_type, ["Na", "Cl"])
    return [metal, "O", "H"] + ions


def parse_constraints(args: argparse.Namespace) -> tuple[list, list, list]:
    """Parse spring constraint arguments."""
    metal_water_spring_constraints = []
    metal_ion_spring_constraints = []
    spring_constraints = []
    ...
    if args.spring_metal_ion:
        for ion_elem, count_str, dist_str in args.spring_metal_ion:
            count = count_str if count_str == "all" else int(count_str)
            distance = float(dist_str)
            metal_ion_spring_constraints.append(
                MetalIonSpringConstraint(ion_elem, count, distance, k_spring=args.k_spring)
            )
    ...
    return metal_water_spring_constraints, metal_ion_spring_constraints, spring_constraints


def handle_command(args: argparse.Namespace) -> int:
    """Handle the spring-metal-salt-water generation command."""
    validate_args(args)

    if args.dry_run:
        logger.info("Dry run - would generate spring-restrained metal-salt-water interface with:")
        ...
        if args.spring_metal_ion:
            for c in args.spring_metal_ion:
                logger.info(
                    f"  Metal-ion spring: {args.metal}-{c[0]} x{c[1]} -> {c[2]} A (K={args.k_spring})"
                )
        if args.spring:
            for c in args.spring:
                logger.info(f"  Spring: {c[0]}-{c[1]} x{c[2]} -> {c[3]} A (K={args.k_spring})")
        return 0
    ...
    try:
        ...
        params = SpringMetalSaltWaterParameters(
            ...
            metal_ion_spring_constraints=metal_ion_spring_constraints,
        ...
        generator = SpringMetalSaltWaterGenerator(params)

        if not getattr(args, "quiet", False):
            logger.info("Generating spring-restrained metal-salt-water interface...")
