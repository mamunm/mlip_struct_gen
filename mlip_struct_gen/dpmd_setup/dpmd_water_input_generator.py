...
class DPMDWaterInputGenerator:
    ...
    def _write_header(self, f):
        ...
        f.write("# LAMMPS input script for DeepMD water simulation\n")
        f.write(
            f"# Generated by mlip-struct-gen on {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
        )
    ...
    def _write_dpmd_potential(self, f):
        ...
        f.write(
            "plugin load /Users/osman_mbp/Osman_Macbook_Pro/Electrolyte_Microenvironment/MLIP_Structure_Generation/mlip_struct_gen/.venv/lib/python3.11/site-packages/deepmd/lib/libdeepmd_lmpplugin.so\n"
        )
        f.write("\n")
    ...
    def _write_setup(self, f):
        ...
        f.write(
            f"velocity all create {self.parameters.temperature} {self.parameters.seed} dist gaussian\n"
        )
        f.write("\n")
    ...
    def _write_water_analysis_setup(self, f):
        ...
        if params.compute_msd:
            ...
            f.write("compute msd_O oxygen msd com yes average yes\n")
            f.write("compute msd_H hydrogen msd com yes average yes\n")
            f.write("\n")
        ...
        if params.compute_hbonds:
            ...
            f.write(
                f"# H-bond criteria: O-O < {params.hbond_cutoff} Å, O-H-O angle > {180 - params.hbond_angle}°\n"
            )
            f.write("\n")
    ...
    def _write_equilibration(self, f):
        ...
        if self.parameters.ensemble == "NVE":
            ...
            f.write(
                f"fix nvt_eq all nvt temp {self.parameters.temperature} {self.parameters.temperature} {self.parameters.thermostat_damping}\n"
            )
            ...
            f.write(
                f"fix npt_eq all npt temp {self.parameters.temperature} {self.parameters.temperature} {self.parameters.thermostat_damping} "
            )
    ...
    def _write_production(self, f):
        ...
        if self.parameters.ensemble == "NVE":
            ...
            f.write("# NVT ensemble with Nosé-Hoover thermostat\n")
            f.write(
                f"fix nvt_prod all nvt temp {self.parameters.temperature} {self.parameters.temperature} {self.parameters.thermostat_damping}\n"
            )
        elif self.parameters.ensemble == "NPT":
            f.write("# NPT ensemble with Nosé-Hoover thermostat and barostat\n")
            pressure_bar = self.parameters.pressure * 1.01325
            f.write(
                f"fix npt_prod all npt temp {self.parameters.temperature} {self.parameters.temperature} {self.parameters.thermostat_damping} "
            )
            f.write(f"iso {pressure_bar} {pressure_bar} {self.parameters.barostat_damping}\n")
        ...
        f.write(
            "fix ave_props all ave/time 1 1000 1000 c_myTemp c_myPE c_myKE file properties.txt\n"
        )
    ...
    def _write_water_analysis_output(self, f):
        ...
        if params.compute_msd:
            ...
            f.write(
                f"fix msd_out all ave/time {params.msd_sample_freq} 1 {params.msd_sample_freq} "
            )
        ...
        if params.compute_rdf:
            ...
            rdf_freq = int(
                self.parameters.production_time / self.parameters.timestep * 100
            )  # Every 1% of production
            f.write(
                f"fix rdf_OO_ave all ave/time 1 {rdf_freq} {rdf_freq} c_rdf_OO[*] file rdf_OO_ave.dat mode vector ave running\n"
            )
            f.write(
                f"fix rdf_OH_ave all ave/time 1 {rdf_freq} {rdf_freq} c_rdf_OH[*] file rdf_OH_ave.dat mode vector ave running\n"
            )
    ...
    **kwargs,
    ...
    if use_water_parameters:
        parameters = DPMDWaterInputParameters(
            dpmd_model_path=dpmd_model_path, lammps_data_file=lammps_data_file, **kwargs
        )
    else:
